...
export const checkBudgetAlerts =
  inngest.createFunction(
    ...
    async ({ step }) => {
      const budgets = await step.run("fetch", async () =>
        db.budget.findMany({
          include: { user: true },
        })
      );
        ...
        await step.run(`budget-${budget.id}`, async () => {
          ...
          if (percent >= 80) {
            await sendEmail({
              ...
              subject: `⚠️ Budget Alert — ${Math.round(percent)}% used`,
              react: EmailTemplate({
...
async function generateFinancialInsights(stats, month) {
  ...
  const prompt = `
You are a financial advisor. Analyze this data for ${month}:
...
Return ONLY a JSON array with exactly 3 short insight strings.
...
`;
  ...
  try {
    ...
    return [
      "Track your expenses carefully this month.",
